        ...
        public static YLS Import(String wat)
        {
            ...
            using (StreamReader str = File.OpenText(wat))
            {
                ...
                while (!str.EndOfStream)
                {
                    ...
                    {
                        line[3] = line[3].Replace("_GPIO", " GPIO").Replace("02-11-15 GPIO", "9.5.0-22");
                    }

                    if (titl.id == 0x400102002CA00) 
                    {
                        line[3] = line[3].Replace("_JPN", " JPN").Replace("10-02-14 JPN", "9.1.0-20J");
                    }

                    if(line[1] == "TWN")
                    {
                        line[3] = line[3].Replace("08-18-16_TWN_BeginScanning", "11.0.0-33T");

                         /*
                          * Wyatt
                          * Probably not actually 11.3.0-36T, but I am not sure what to call it. It was the first update to new3ds after 11.3.0.36
                          * I am following the example above with '11.0.0-33T', which was chronologically released after 11.0.0-33.
                          */
                        line[3] = line[3].Replace("N/A", "11.3.0-36T");	
                    }
                    

                    Match mat = Regex.Match(line[3], @"(\d+)\.(\d+)\.(\d+)-(\d+)");

                    /* wyatt: writing all parsed data to console for easier diagnoses */
                    Console.Write("0 " + line[0] + "\n");
                    Console.Write("1 " + line[1] + "\n");
                    Console.Write("2 " + line[2] + "\n");
                    Console.Write("3 " + line[3] + "\n");
